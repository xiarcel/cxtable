...
/*this class handles creating one of two types of "Server Sockets".
The first, is a primary "xRegistry" type, that requires posting it via FTP
to the appropriate spot.  The second is for the xMobileServer (peer) type
of ServerSocket, or other secondary ones in the Registry.  It takes
a xServerListener reference, and passes it the Sockets as they connect*/
  ...
  import java.net.*;
   import java.io.*;
	...
	private String http="";
      ...
      {
         xsl=x; xlin=new xLinlyn(site,user,pw,di);
         file=fi;
         withlinlyn=true;
      }
	public xServSocket(String http_data, String sname, xServerListener x)
	{
	xsl=x;http=http_data;pservname=sname;
	withlinlyn=false;
	as_php=true;
	}
      ...
      public void run()
      {
         if (withlinlyn==true)
         {
			...
			catch(Exception e){System.out.println("Error erasing/php!");}
			}
         ...
         try{
            ...
            {	
               server = ss.accept();    
               if(server !=null){xsl.add(server);System.out.println("added connect");}
               else{System.out.println("Received null socket");}
               server=null;
